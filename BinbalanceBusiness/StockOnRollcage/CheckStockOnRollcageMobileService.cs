...
using System.Linq;
using System.Text;
using static BinbalanceBusiness.Stock.ViewModels.CheckStockOnRollcageMobileViewModel;

namespace BinbalanceBusiness.Stock
{
    public class CheckStockOnRollcageMobileService
    {
        private BinbalanceDbContext db;
        private MasterDbContext dbMaster;

        public CheckStockOnRollcageMobileService()
        {
            db = new BinbalanceDbContext();
            dbMaster = new MasterDbContext();
        }

        public CheckStockOnRollcageMobileService(BinbalanceDbContext db)
        {
            this.db = db;
        }

        public actionResultViewModel mobileFilterByProduct(CheckStockOnRollcageMobileViewModel model)
        {
            try
            {
                ...
                var join_sale = (from bin in queryGroup
                                 ...
                                 select new CheckStockOnRollcageMobileViewModel
                                 {
                ...
                var result = new List<CheckStockOnRollcageMobileViewModel>();
                int addNumber = 0;

                foreach (var item in ITem)
                {
                    var unit = get_conversion.Where(c => c.product_Index == item.product_Index).FirstOrDefault();
                    var resultItem = new CheckStockOnRollcageMobileViewModel();
                    addNumber++;
        ...
        public actionResultViewModel mobileFilterByLocation(CheckStockOnRollcageMobileViewModel model)
        {
            var actionResult = new actionResultViewModel();
            try
            {
